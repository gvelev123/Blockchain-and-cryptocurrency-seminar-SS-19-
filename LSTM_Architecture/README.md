...
```yaml
Name of QuantLet: Long short-term memory (LSTM) Neural Network: Architecture
...
def multi_step_LSTM(n_steps_in=24, n_features=1,n_steps_out=24,nr_neurons=100,lr=0.01,epochs=1):
   ...
   # model.add(CuDNNLSTM(nr_neurons,return_sequences=True,bias_initializer='ones',kernel_initializer=keras.initializers.he_normal(seed=42))) 
    
    model.add(CuDNNLSTM(nr_neurons ,bias_initializer='ones',kernel_initializer=keras.initializers.he_normal(seed=42)))
   
   # model.add(Dense(nr_neurons,bias_initializer='ones',kernel_initializer=keras.initializers.he_normal(seed=42)))
